...
const normalizeVehicle = (vehicle: Vehicle, index: number): Vehicle => {
  ...
  return {
    ...
    evidence: (vehicle.evidence ?? []).filter(Boolean).slice(0, 7),
  };
};
...
- 각 차량은 후보를 정하기 전에 아래 순서를 반드시 지켜 독립적으로 관찰하세요.
  ...
  7. 전체 외관: 차체 비율, 실루엣, 필러, 휠아치, 측면 캐릭터 라인과 후면 형상을 종합하세요.
- evidence에는 위 7개 항목을 같은 순서로 정확히 7개 작성하고, 각 문자열을 "로고:", "범퍼:", "헤드램프:", "라디에이터 그릴:", "루프:", "포그 램프:", "전체 외관:"으로 시작하세요.
- 화면에서 확인할 수 없는 항목은 특징을 만들어내지 말고 해당 항목에 "관찰 불가"와 그 이유를 작성하세요.
- 차종 후보는 한 가지 특징만으로 결정하지 말고, 7단계에서 관찰한 특징들의 일치점과 충돌점을 함께 비교하여 정하세요.
...
- {"summary":"분석 요약","vehicles":[{"id":"VEH_01","description":"차량 설명","confidence":0,"color":"색상","bodyType":"차체 유형","box":{"left":0,"top":0,"width":0,"height":0},"candidates":[{"name":"예상 차종","confidence":0}],"evidence":["로고: 관찰 내용","범퍼: 관찰 내용","헤드램프: 관찰 내용","라디에이터 그릴: 관찰 내용","루프: 관찰 내용","포그 램프: 관찰 내용","전체 외관: 관찰 내용"]}]}
`;
